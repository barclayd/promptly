...
The 26 tools cover connection information, content search, full prompt/composer/snippet reads, version/model discovery, prompt/composer creation and editing, validation and previews, composer markup generation, dedicated publication, activity inspection/restoration, and LLM testing. Testing adds `test_prompt`, `test_snippet`, `test_composer`, `compare_tests`, and `get_test_result`; see [testing contracts and examples](./mcp-testing.md). Snippet authoring and deletion tools remain outside the MCP release. Published prompts also appear through native MCP `prompts/list` and `prompts/get`.
...
Each connection is bound to its user, sole workspace membership, client, and consented scopes. Every request checks current membership and permissions. Consent offers read, draft editing, and publishing; draft editing is the default. Run tests is a separate checkbox, off by default, granting `mcp:run` alongside any content permission. Setup instructions request all four scopes; existing grants do not acquire additional permissions automatically. Reconnect and enable Run tests to add testing to an existing connection.
...
| `MCP_AUTHORING_ENABLED` | `true` | Exposes authoring/testing tools, full discovery scopes and native published prompts |
...
Testing requires migration `0029_mcp_test_runs.sql` before deploying the testing Worker. It adds connection-scoped test claims and retained results; it does not modify content tables or existing grants. Apply remotely as part of deployment, then deploy the Worker, refresh the client tool catalog, and reconnect with Run tests selected. No production migration or paid-provider smoke test was performed during local implementation. The minute cron removes expired completed/failed test results; interrupted claims remain to prevent automatic repeat charges.
...
The testing extension passed **279 E2E tests without retries**, lint (existing warnings only), typecheck, production build and Worker dry run. Tests cover real MCP/SDK requests against intercepted provider responses, exact versions and shared comparison inputs, pinned dependencies and unpublished draft dependencies, separate testing consent, revocation during generation, bounded output/concurrency, cancellation, sanitized failures, and durable replay without extra provider calls. Mobile consent/settings were inspected in light and dark modes with Playwright Chromium; Chrome DevTools MCP was unavailable. An intermittent login-fixture hydration race was removed by authenticating shared test setup through the real login action; dedicated browser login tests remain intact and pass. No paid provider calls or production migration/deployment were performed for this extension.
